...
ApplicationWindow {
    title: qsTr("DelegateModel Sample 3")
    width: 640
    height: 600
    menuBar: MenuBar {
        Menu {
    ...
    Item {
        ...
        Column{
            ...
            Text {
                ...
                text: "In this example we have a single model with 12 entries and via a Delegate Model Group we are filtering that data so that we can display a sub-set of the data in the view depending on which group the user selects"
            }
            Row {
                anchors.left: parent.left
                anchors.right: parent.right
                ...
                RadioButton {
                    ...
                    onCheckedChanged: {
                        if ( checked ) {
                            theDelegateModel.delegate = detailsDelegate
                            theDelegateModel.filterOnGroup =  "detailsField";
                        }
                    }
                }
                RadioButton {
                    ...
                    onCheckedChanged: {
                        if ( checked ) {
                            theDelegateModel.delegate = displayDelegate
                            theDelegateModel.filterOnGroup =  "displayField";
                        }
                    }
                }
                RadioButton {
                    ...
                    onCheckedChanged: {
                        if ( checked ) {
                            theDelegateModel.delegate = valueDelegate
                            theDelegateModel.filterOnGroup =  "evenNumbers";
                        }
                    }
                }
            }
